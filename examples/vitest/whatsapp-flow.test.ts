...
function buildApp(mock: () => Wamock) {
  return {
    async handleWebhook(rawBody: string, signature: string | undefined) {
      ...
      await mock().send({
        to: message.from,
        text: `You said: ${message.text.body}`,
      });
    },
  };
}
...
beforeEach(async () => {
  ...
  mock = await createWamock({
    ...
    onWebhook: (delivery) =>
      app.handleWebhook(delivery.body, delivery.signature),
  });
});
...
describe("my WhatsApp integration", () => {
  ...
  it("cannot send free-form text once the 24h window closes", async () => {
    ...
    await expect(
      mock.send({ to: CUSTOMER, text: "still there?" }),
  ...
  it("fails when the template was approved in a different language", async () => {
    ...
    await expect(
      mock.send({
        to: CUSTOMER,
        template: { name: "recordatorio_24h", language: "en_US" },
      }),
  ...
  it("backs off and recovers from a rate limit", async () => {
    ...
    await expect(
      mock.send({ to: CUSTOMER, text: "one" }),
    ).rejects.toMatchObject({ code: 130429 });
    await expect(
      mock.send({ to: CUSTOMER, text: "two" }),
    ).rejects.toMatchObject({ code: 130429 });
    await expect(
      mock.send({ to: CUSTOMER, text: "three" }),
