...
const valueOf = (raw: { body: string }) => JSON.parse(raw.body).entry[0].changes[0].value
...
describe('the full lifecycle, with no network', () => {
  it('walks inbound → reply → statuses → expiry → 131047 → template → OK', async () => {
    ...
    await post(`/v19.0/${waba}/message_templates`, {
      ...
      language: 'es_MX',
      category: 'UTILITY',
      components: [{ type: 'BODY', text: 'Hola {{1}}, tu pedido va en camino.' }],
    })
    ...
    expect(statuses.map((v) => v.statuses[0].status)).toEqual(['sent', 'delivered'])
